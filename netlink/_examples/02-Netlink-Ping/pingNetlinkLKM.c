...
#include <net/sock.h>               // Sockets
#include <linux/string.h>           // Kernel string functions: memset, memcpy. NB: <string.h> is for user space.
#include <linux/kernel.h>           // Kernel print functions : snprintf
#include "netLinkKernelUtils.h"     // Utility

// Global Variables
//
static struct sock *nl_sk = NULL;   // Netlink socket pointer

// Receiver Function 
//
// Handles data received over netlink socket from user space.
//
// SKB - Socket Kernel Buffer
//
// The SKB is a unified data structure for packaging the (network) data being transported:
//
//   1. Between kernel subsystems.
//   2. Kernelspace to userspace.
//   3. Userspace to kernelspace 
// 
// The internal member of this socket buffer is not accessed directly, but rather kernel APIs
// provide setters/getters for this purpose.
//
//
// When message comes over netlink socket from userspace, the kernel packages the data in 
// 'sk_buff' data structures and invokes this function with pointer to that skb.
//
...
asdf
    ...
    if (nlh_recv->nlmsg_flags & NLM_F_ACK) {
        ...
        // Send reply back to user space process
        memset(kernel_reply, 0 , sizeof(kernel_reply));

        // `nsprintf' (linux/kernel.h) can be used to 
        snprintf(kernel_reply, sizeof(kernel_reply), 
                "Msg from Process %d has been processed by kernel", nlh_recv->nlmsg_pid);

        // Get a new sk_buff with an empty Netlink hdr already appended before payload space
        // 
        // i.e. skb_out->data should be be pointer to following message structure : 
        // 
        //      +-------------+--------------+
        //      | Netlink Hdr |   Payload    |
        //      +-------------+--------------+
